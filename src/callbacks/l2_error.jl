...
Callback that computes the L²(Ω) error between the numerical and exact solutions
at each time step.
...
- `c_errors::Vector{T}`: L² errors for the thermal field (size `nt`)
...
function affect!(
    ...
    cb.c_errors[state.n + 1] = L2_error(
        (x, y) -> input_data.θ(x, y, state.t), state.c, mesh2D, dof_map_m₂, quad)
