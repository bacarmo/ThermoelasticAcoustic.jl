"""
    compute_initial_state(
        lhs_wave, lhs_thermal, lhs_acoustic,
        dof_map_wave, dof_map_thermal, dof_map_acoustic,
        mesh2D, mesh1D, quad, input_data)

Compute the initial [`FEMState`](@ref) for the wave-thermal-acoustic coupled problem.

Wave (v⁰, d⁰) and thermal (c⁰) fields are obtained via H₀¹ projection on `mesh2D`;
...
function compute_initial_state(
    ...
    c⁰ = zeros(eltype(lhs_thermal), dof_map_thermal.m)
    compute_c⁰!(c⁰, lhs_thermal, input_data, mesh2D, dof_map_thermal, quad)
    ...
    return FEMState(0, zero(eltype(lhs_wave)), v⁰, d⁰, c⁰, r⁰, z⁰)
end
...
"""
    compute_c⁰!(c⁰, lhs_mat, input_data, mesh, dof_map, quad)

Compute the initial thermal field `c⁰` via H₀¹ projection.

# Arguments
- `c⁰::AbstractVector`: Initial thermal coefficients (in-place, length `dof_map.m`)
- `lhs_mat`: Stiffness matrix (m₂ × m₂); factorized internally via Cholesky
- `input_data`: Structure with gradient fields `∂ₓθ₀`, `∂ᵧθ₀`
- `mesh::CartesianMesh{2}`: 2D mesh for the thermal field
- `dof_map::DOFMap`: DOF mapping with `m` free DOFs
- `quad::QuadratureSetup`: Precomputed quadrature data
"""
function compute_c⁰!(c⁰, lhs_mat, input_data, mesh, dof_map, quad)
    vec₁ = similar(c⁰)
    vec₂ = similar(c⁰)
    factorized_lhs_mat = cholesky(lhs_mat)

    projection_H01_2d!(c⁰, input_data.∂ₓθ₀, input_data.∂ᵧθ₀,
        factorized_lhs_mat, mesh, dof_map, quad, vec₁, vec₂)
