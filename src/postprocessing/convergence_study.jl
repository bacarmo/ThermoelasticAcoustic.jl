...
Stores one value per solution field of the thermo-wave-acoustic system at each
...
function RefinementData{T}(n::Int) where {T <: AbstractFloat}
    RefinementData{T}(zeros(T, n), zeros(T, n), zeros(T, n), zeros(T, n), zeros(T, n))
end
...
function print_convergence_table(results::ConvergenceResults)
    ...
    println(sep)
    @printf("  %5s  %7s  %7s  %10s  %6s  %10s  %6s  %10s  %6s  %10s  %6s  %10s  %6s\n",
        "Nx", "log₂h", "log₂τ",
        "L∞L²(v)", "rate",
        "L∞L²(d)", "rate",
        "L∞L²(c)", "rate",
    ...
    for i in eachindex(results.Nx)
        @printf("  %5d  %7.2f  %7.2f  %10.2e  %6.3f  %10.2e  %6.3f  %10.2e  %6.3f  %10.2e  %6.3f  %10.2e  %6.3f\n",
            results.Nx[i],
            log2(results.h[i]),
            log2(results.τ[i]),
            results.errors.v[i], results.rates.v[i],
            results.errors.d[i], results.rates.d[i],
            results.errors.c[i], results.rates.c[i],
...
function run_convergence_study(
    ...
    for i in eachindex(Nx_values)
        ...
        errors.c[i] = maximum(cb.c_errors)
...
function compute_rates!(
    ...
    for (e, r) in zip(
        (errors.v, errors.d, errors.c, errors.r, errors.z),
        (rates.v, rates.d, rates.c, rates.r, rates.z))
        for i in 2:length(e)
            r[i] = log2(e[i - 1] / e[i])
        end
    end
