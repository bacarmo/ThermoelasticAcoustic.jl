...
- `c::Vector{T}`: FE coefficients vector of the thermal solution Θⁿ(x,y) (size m₂)
...
- `c_errors::Vector{T}`: L² errors for the thermal field
...
function L2ErrorCallback(tspan::StepRangeLen{T}) where {T}
    return L2ErrorCallback(
        zeros(T, length(tspan)), zeros(T, length(tspan)), zeros(T, length(tspan)),
        zeros(T, length(tspan)), zeros(T, length(tspan)))
end

"""
    apply!(cb, state, nel_per_dim, element_side_lengths, dof_map_m₁, dof_map_m₂, dof_map_m₃, quad, input_data)
...
function apply!(cb::L2ErrorCallback{T}, state::State{T},
        ...
        dof_map_m₁::DOFMap, dof_map_m₂::DOFMap, dof_map_m₃::DOFMap,
    ...
    cb.d_errors[state.n + 1] = error_L2(
        ...
        dof_map_m₁, nel_per_dim, element_side_lengths,
        quad.W, quad.xP, quad.yP, quad.φP)
    cb.c_errors[state.n + 1] = error_L2(
        (x, y) -> input_data.θ(x, y, state.t), state.c,
        dof_map_m₂, nel_per_dim, element_side_lengths,
        quad.W, quad.xP, quad.yP, quad.φP)
...
- `c::Vector{Vector{T}}`: FE coefficient history; entry `n+1` holds the solution at `tspan[n+1]`
...
function SolutionCallback(tspan::StepRangeLen{T}) where {T}
    return SolutionCallback(tspan, Vector{Vector{T}}(), Vector{Vector{T}}(),
