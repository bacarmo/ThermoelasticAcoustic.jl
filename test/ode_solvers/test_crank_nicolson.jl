...
@testitem "compute_JH!: CrankNicolson solver, JH matrix assembly" begin
    using ThermoelasticAcoustic: assembly_global_matrix_DF, assembly_global_matrix_DG,
                                 build_cache, CartesianMesh, compute_JH!, compute_Q!,
    ...
    # Test τ = α = 0: JH[1:m₁, 1:m₁] = 2M_m₁xm₁ (all τ-dependent terms vanish)
    ...
    @test cache.JH[1:m₁, 1:m₁] ≈ Matrix(cache.M_m₁xm₁_vs2)

    # Test α=1: JH[1:m₁, 1:m₁] = Q[1:m₁,1:m₁] + τα JG + (τ²/2) JF
    ...
    JH_block_expected = Matrix(cache.Q[1:m₁, 1:m₁])
    JH_block_expected[1:m₃, 1:m₃] += Matrix(JG)
    JH_block_expected[1:m₁, 1:m₁] += Matrix(JF)

    @test cache.JH[1:m₁, 1:m₁] ≈ JH_block_expected

    # Test time-invariant off-diagonal blocks are preserved
    @test cache.JH[1:m₁, (m₁ + 1):end] ≈ Matrix(τ * matrices.A_m₁xm₂)
    @test cache.JH[(m₁ + 1):end, 1:m₁] ≈ Matrix(τ * matrices.A_m₂xm₁)

    # Test bottom-right m₂×m₂ block: 2M_m₂×m₂ + τβ K_m₂×m₂ + τdβ (K_m₂×m₂ ĉⁿ) bᵀ
